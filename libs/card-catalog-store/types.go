...
// in DynamoDB. The service is standalone — data enters exclusively through
// the write API, never from an upstream source.
//
// Data-model rules:
//   - prices live on card variants, overwritten in place; nothing is stored
//     on the card row itself
//   - set card counts are maintained transactionally alongside card writes
//     (see docs/decisions/0002-dynamodb-single-table.md)
...
type CardVariant struct {
	ID    string   `json:"id" dynamodbav:"id"` // GUID
...
type SetSummary struct {
	...
	CardCount   int       `json:"cardCount"`   // maintained transactionally with card writes
...
type CardDetail struct {
	...
	TCGPlayerID *int          `json:"tcgplayerId"` // TCGplayer product id; nil when not listed there
	Language    string        `json:"language"`    // ISO 639 alpha-3; immutable
	GameID      string        `json:"gameId"`      // game GUID
	Name        string        `json:"name"`
	Number      *string       `json:"number"`
	Rarity      *string       `json:"rarity"`
